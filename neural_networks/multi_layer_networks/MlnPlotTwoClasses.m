function MlnPlotTwoClasses(sln, X, T)
%MlnPlotTwoClasses Plot descrimination line for a two-class problem
...
%T being the true class vector (a row vector).
    ...
    ym = ym(:);
    grid = [xm ym]';
    Z = MlnFwdLinear(sln, grid);
    Z = reshape(Z, length(y), length(x));
    v = [ 0 0 ]; %[-0.5 0 0.5]; %[0.1 0.5 0.9];
    [c, h] = contour(x, y, Z, v);
    title('Neural Network for two classes')
    set(h, 'linewidth', 3)
    clabel(c, h);
